...
namespace NMib::NConcurrency
{
			...
			if constexpr (mc_bIsFuture || mc_bIsAsyncGenerator)
			{
				...
				try
				{
					...
					if constexpr (mc_bIsFuture)
					{
						...
						DMibFastCheck
							(
								PromiseThreadLocal.m_pUsePromise
								||
								(
									!PromiseThreadLocal.m_pUsePromise
						...
						{
							DMibFastCheck(m_pActorInternal); // We need State to be alive here
							DMibFastCheck(PromiseThreadLocal.m_pUsePromise == &m_Promise);
							Future.f_OnResultSet
								(
									[
										Promise = fg_Move(m_Promise)
#if DMibConfig_Concurrency_DebugActorCallstacks
										, Callstacks = fg_Move(Callstacks)
#endif
									]
									(TCAsyncResult<t_CRet> &&_Result) mutable
									{
#if DMibConfig_Concurrency_DebugActorCallstacks
										_Result.m_Callstacks = fg_Move(Callstacks);
#endif
	...
	void NPrivate::TCFutureCoroutineContextShared<t_CReturnType>::CConcurrentRunQueueEntryImpl::f_Call(CConcurrencyThreadLocal &_ThreadLocal)
	{
		...
		[[maybe_unused]] NStorage::TCSharedPointer<NPrivate::TCPromiseData<t_CReturnType>> pPromiseData
			{
		...
		auto RestoreStates = CoroutineContext.f_ResumeInitialSuspend(_ThreadLocal.m_SystemThreadLocal);
	...
	inline_never void NPrivate::TCFutureCoroutineContextShared<t_CReturnType>::fp_CleanupDirectResume()
	{
		...
		DMibFastCheck(PromiseData.m_PromiseRefCount.f_Get() == -1);
#else
		DMibFastCheck(PromiseData.m_PromiseRefCount.f_Load() == -1);
#endif
		...
		)
	{
		...
		else
		{
			...
			auto pPromiseData = fg_TempCopy(Result.f_Unsafe_PromiseData());
			auto &PromiseData = *pPromiseData;
			pPromiseData.f_Detach();
