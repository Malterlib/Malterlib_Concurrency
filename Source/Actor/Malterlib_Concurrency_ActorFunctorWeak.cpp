...
namespace NMib::NConcurrency
{
	...
	TCFuture<void> CActorFunctorWeakCoalesced::operator ()() const
	{
		...
		// the disarm at the target's start, so reopen the gate here; a redundant disarm after a
		// normal run is harmless
		TCPromiseFuturePair<void> Pair;
		fg_Move(CallFuture).f_OnResultSet
			(
				[pGate = Control.m_pGate, Promise = fg_Move(Pair.m_Promise)](TCAsyncResult<void> &&_Result) mutable
				{
					if (!_Result)
