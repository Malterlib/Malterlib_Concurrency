...
namespace NMib::NConcurrency::NPrivate
{
	...
	{
		auto KeepAlive = fg_ConsumeFutureOnResultSet<TCAsyncGenerator<t_CReturnType>>
			(
#if DMibEnableSafeCheck > 0
				this
		...
		return g_ActorFunctor
			...
			) / [KeepAlive = fg_Move(KeepAlive), pPromiseData = fg_Move(pPromiseData), bInProgress = false]() mutable -> TCFuture<NStorage::TCOptional<t_CReturnType>>
			{
				if (bInProgress)
				...
				if (Result)
				{
					...
					g_Dispatch /
						[
							KeepAlive = TCFutureCoroutineKeepAliveImplicit<NStorage::TCOptional<t_CReturnType>>(fg_TempCopy(pPromiseData))
