...
//#define DMibConcurrency 1
	...
	void CConcurrencyManager::f_Stop()
	{
		...
		// owner. Every socket is actor-owned, and f_BlockOnDestroy waited for every user actor to
		// go while the loops still ran: an actor holding a binding kept the wait going while its
		// queue's thread processed the enable that came with the binding, so a loop that reaches
		// this unclaimed never had anything registered. The loop's destruction checks that.
		// Destroying here is what keeps a non-process-lifetime manager from leaking every
		// loop's descriptors
		for (umint Prio = 0; Prio < EPriority_Max; ++Prio)
		{
...
#	endif
	...
	// Every 64th entry a thread processes, a job of its queue or a message of an actor's mailbox
	// alike, looks up from the drain it is in: a long drain never returns to the pool thread
	// loop, so the work queued behind it is offered to idle threads, and the loop bound to the
	// queue is polled, since a socket bound to it would otherwise hear nothing for as long as
	// the drain goes on. The offer's target is the caller's: a drain that has proven long offers
	// everything, a short one keeps the batching target, since its job boundary is imminent and
	// shipping a lone peer resume would put a wakeup on the critical path of every exchange
...
#else
		(void)_OfferTargetSize;
#endif

		if (_Queue.m_bIoLoopParkActive) [[unlikely]]
			_Queue.m_pIoLoop->f_PollAndDispatch();
	}
					...
					if ((OriginalWorking & gc_ProcessingMask) == 0)
					{
						...
						while (bDoneSomething)
						{
							...
							while (auto *pJob = _Queue.m_JobQueueLocal.f_PopFirst())
							{
								...
								// A job counts as one processed entry on this thread; an actor's
								// mailbox drain counts its messages into the same tally, so the
								// checkpoint comes every 64 entries whichever loop is draining.
								// The checkpoint offers the excess itself, so the threshold offer
								// is for the jobs in between
								if ((++ThreadLocal.m_nProcessedEntries & 63) == 0)
									fp_DrainCheckpoint(_Queue, DMibConfig_Concurrency_LocalQueueTargetSize);
#if DMibConfig_Concurrency_LocalFirstScheduler && DMibConfig_Concurrency_LocalFirstDistribution
								else if (_Queue.m_JobQueueLocal.m_nEntries >= DMibConfig_Concurrency_LocalQueueTargetSize * 2)
									fp_OfferExcessWork(_Queue, true);
#endif
