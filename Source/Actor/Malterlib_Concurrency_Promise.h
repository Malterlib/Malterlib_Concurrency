...
namespace NMib::NConcurrency
{
	...
	struct CFutureCoroutineContext
	{
		...
		using FGetKeeptAlivePromiseData = NFunction::TCFunctionSmallMovable<NPrivate::CPromiseDataBase *()>;
		using FGetVirtualKeepalive = FGetKeeptAlivePromiseData (CFutureCoroutineContext &_Context, TCActor<CActor> &&_Actor);
		...
		void f_HandleAwaitedException(CConcurrencyThreadLocal &_ThreadLocal, FGetVirtualKeepalive *_fGetKeepAlive, NException::CExceptionPointer &&_pException);
		FDeliverExceptionResult f_PrepareExceptionResult(FGetVirtualKeepalive *_fGetKeepAlive, TCActor<CActor> &&_Actor);
...
#endif
		;
		NContainer::TCVector<NFunction::TCFunctionMovable<void ()>> f_Resume(CSystemThreadLocal &_ThreadLocal, FGetVirtualKeepalive *_fGetKeepAlive, bool &o_bAborted);

		void f_InitialSuspend(CSystemThreadLocal &_ThreadLocal);
		NContainer::TCVector<NFunction::TCFunctionMovable<void ()>> f_ResumeInitialSuspend(CSystemThreadLocal &_ThreadLocal);
		void f_Abort();
...
namespace NMib::NConcurrency::NPrivate
{
	...
	struct TCRunProtectedFutureHelper
	{
		...
		mark_no_coroutine_debug TCUnsafeFuture<t_CReturnValue> operator () (tf_FToRun &&_ToRun, tfp_CParams && ...p_Params) const;
	...
	struct TCFutureCoroutineContextShared : public CFutureCoroutineContext
	{
		...
		static FGetKeeptAlivePromiseData fs_GetVirtualKeepAlive(CFutureCoroutineContext &_Context, TCActor<CActor> &&_Actor);
	...
	struct CPromiseDataBase
	{
...
#else
		using CPromiseRefCount = NAtomic::TCAtomic<int32>;
#endif

		inline_always NAtomic::EMemoryOrder f_MemoryOrder(NAtomic::EMemoryOrder _Default) const;

		bool f_PromiseRefCountDecrease(DIfRefCountDebugging(NStorage::CRefCountDebugReference &_Reference));
		void f_PromiseRefCountIncrease(DIfRefCountDebugging(NStorage::CRefCountDebugReference &_Reference));
		...
		struct CCanOnlyBeSetBeforeSuspend
		{
			...
			uint8 m_bNeedToRefcountPromise:1 = false;
	...
	struct TCPromiseData final : public CPromiseDataBase
	{
		...
		TCPromiseData(TCPromiseDataInit<t_CReturnValue> &&_Init);
		TCPromiseData(FOnResult &&_fOnResult);
		TCPromiseData(CPromiseConstructDiscardResult);
		~TCPromiseData();
...
namespace NMib::NConcurrency
{
	...
	struct [[nodiscard("You need to co_await or forward the result to a functor with > operator")]] DMibConcurrencyInstrumentFunctionEnter TCFuture
	{
		...
		TCFuture(CInitializationValue &&_ReturnValue);
		TCFuture(CInitializationValue const &_ReturnValue);
	...
	private:
		...
		TCFuture(NStorage::TCSharedPointer<NPrivate::TCPromiseData<t_CReturnValue>> const &_pData, bool _bIgnoreFutureGotten);
	...
	public:
		...
		TCPromise(CPromiseConstructEmpty, CConcurrencyThreadLocal &_ThreadLocal);
		TCPromise(CPromiseConstructNoConsume, CConcurrencyThreadLocal &_ThreadLocal = fg_ConcurrencyThreadLocal());
		TCPromise(CPromiseConstructDiscardResult);
		TCPromise(CPromiseConstructDiscardResult, CConcurrencyThreadLocal &_ThreadLocal);
		TCPromise(FOnResult &&_fOnResult);
		TCPromise(FOnResult &&_fOnResult, CConcurrencyThreadLocal &_ThreadLocal);
		...
		mark_no_coroutine_debug NStorage::TCSharedPointer<NPrivate::TCPromiseData<t_CReturnValue>> f_MoveData
			(
#if DMibConfig_RefCountDebugging
				NStorage::CRefCountDebugReference &_DestinationReference
#endif
		...
		mark_no_coroutine_debug TCFuture<t_CReturnValue> fp_FutureIgnoreFutureGotten();
	...
	private:
#if DMibConfig_RefCountDebugging
		NStorage::CRefCountDebugReference mp_Reference;
#endif
		NStorage::TCSharedPointer<NPrivate::TCPromiseData<t_CReturnValue>> mp_pData;
	};
