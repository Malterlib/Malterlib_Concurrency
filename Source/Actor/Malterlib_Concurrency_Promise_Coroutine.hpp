...
namespace NMib::NConcurrency
{
	...
	{
		CConcurrencyThreadLocal *pThreadLocal = nullptr;
		auto fGetThreadLocal = [&]() inline_always_lambda -> CConcurrencyThreadLocal &
			{
				if (pThreadLocal)
					return *pThreadLocal;

				pThreadLocal = &fg_ConcurrencyThreadLocal();
		...
		if (m_bRuntimeStateConstructed) [[unlikely]] // Only happens on exceptions, otherwise final_suspend should take care of this
		{
			...
			else if (m_bPreviousCoroutineHandlerConstructed)
			{
				...
				auto &ThreadLocal = fGetThreadLocal().m_SystemThreadLocal;
			...
			{
				auto &ConcurrencyThreadLocal = fGetThreadLocal();
		...
		if (m_pPromiseData->m_BeforeSuspend.m_bAllocatedInCoroutineContext)
		{
			auto &ThreadLocal = fGetThreadLocal().m_SystemThreadLocal.m_PromiseThreadLocal;
			ThreadLocal.f_PushAllocation(m_pPromiseData);
		}
	}
}
...
namespace NMib::NConcurrency::NPrivate
{
	...
	{
		NStorage::TCSharedPointer<TCPromiseData<t_CReturnType>> pPromiseDataShared = fg_ConsumePromise<t_CReturnType, EConsumePromiseFlag_None>
			(
				_ThreadLocal
				, nullptr
#if DMibConfig_RefCountDebugging
	...
	template <typename t_CReturnType>
	auto TCFutureCoroutineContextShared<t_CReturnType>::fs_GetVirtualKeepAlive(CFutureCoroutineContext &_Context, TCActor<CActor> &&_Actor) -> FGetKeeptAlivePromiseData
	{
		auto *pThis = static_cast<TCFutureCoroutineContextShared<t_CReturnType> *>(&_Context);

		return [KeepAlive = pThis->f_KeepAlive(fg_Move(_Actor))]() -> CPromiseDataBase *
			{
				return KeepAlive.mp_pPromiseData.f_Get();
			}
		;
	}
		...
		: mp_pPromiseData(fg_Move(_pPromiseData))
	{
		...
		mp_pPromiseData->f_PromiseRefCountIncrease(DIfRefCountDebugging(m_Reference));
	}
...
#if DMibConfig_RefCountDebugging
		if (mp_pPromiseData)
			mp_pPromiseData->m_PromiseRefCount.f_Move(_Other.m_Reference, m_Reference);
#endif
	}
	...
	TCFutureCoroutineKeepAliveImplicit<t_CReturnType>::~TCFutureCoroutineKeepAliveImplicit()
	{
		...
		auto &PromiseData = *mp_pPromiseData;

		if (!PromiseData.f_PromiseRefCountDecrease(DIfRefCountDebugging(m_Reference)))
			return;

		if (PromiseData.m_AfterSuspend.m_bPendingResult && !PromiseData.m_AfterSuspend.m_bCoroutineValid)
		{
	...
	TCFutureCoroutineKeepAlive<t_CReturnType> TCFutureCoroutineContextShared<t_CReturnType>::f_KeepAlive(TCActor<> &&_Actor)
	{
		...
		if (!pPromiseData->m_BeforeSuspend.m_bNeedToRefcountPromise)
