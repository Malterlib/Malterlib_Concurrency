...
namespace NMib::NConcurrency
{
	...
	uint64 CActorDistributionManagerInternal::fp_QueuePacket(NStorage::TCSharedPointerSupportWeak<CHost> const &_pHost, NStream::CBinaryStorage &&_Data)
	{
		...
		// The packet header is in the local arena and remains patchable until sharing.
	...
	void CActorDistributionManagerInternal::fp_ScheduleSendPacketQueue(NStorage::TCSharedPointerSupportWeak<CHost> const &_pHost)
	{
		...
		DMibLogWarningOrDiscardResult
			(
				fg_ThisActor(m_pThis).f_Bind<&CActorDistributionManager::fp_FlushSendPacketQueue>(NStorage::TCSharedPointerSupportWeak<NPrivate::ICHost>(_pHost))
				, "Mib/Concurrency"
				, "Flushing the distributed actor send packet queue failed"
			)
		;
	}
