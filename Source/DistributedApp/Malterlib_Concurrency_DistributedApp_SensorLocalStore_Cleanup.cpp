...
namespace NMib::NConcurrency
{
	...
	TCFuture<NDatabase::CDatabaseActor::CTransactionWrite> CDistributedAppSensorStoreLocal::CInternal::f_Cleanup(NDatabase::CDatabaseActor::CTransactionWrite &&_WriteTransaction)
	{
		if (m_fCleanup)
			co_return co_await m_fCleanup(fg_Move(_WriteTransaction));

		auto WriteTransaction = co_await m_pThis->self(&CDistributedAppSensorStoreLocal::f_PrepareForCleanup, fg_Move(_WriteTransaction));
		...
		try
		{
			...
			NTime::CTime OldestAllowedReading = NTime::CTime::fs_NowUTC() - NTime::CTimeSpanConvert::fs_CreateDaySpan(m_RetentionDays);
