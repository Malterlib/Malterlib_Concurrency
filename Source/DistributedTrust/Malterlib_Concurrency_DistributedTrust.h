...
namespace NMib::NConcurrency
{
	...
	public:
		...
		using CListenInfo = CDistributedActorTrustManagerInterface::CListenInfo;
		...
		struct COptions
		{
			...
			uint64 m_DefaultSendWindowBytes = 0; // Default bytes in flight; zero leaves the transport default of eight frames.
		...
		TCFuture<NContainer::TCMap<CDistributedActorTrustManager_Address, CListenInfo>> f_EnumListens();
		...
		TCFuture<CHostInfo> f_AddClientConnection(CTrustTicket _TrustTicket, fp64 _Timeout, int32 _ConnectionConcurrency = -1, uint64 _SendWindowBytes = 0);
		TCFuture<CHostInfo> f_AddAdditionalClientConnection(CDistributedActorTrustManager_Address _Address, int32 _ConnectionConcurrency = -1, uint64 _SendWindowBytes = 0);
