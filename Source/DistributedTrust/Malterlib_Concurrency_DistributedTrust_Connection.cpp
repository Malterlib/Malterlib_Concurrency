...
namespace NMib::NConcurrency
{
	...
	// Mutate memory before saving a copy. Database submission order then preserves overlapping setters' changes.
	// Failures intentionally leave memory ahead of persistence or live settings; setting the same value again is a no-op.
	...
	TCFuture<void> CDistributedActorTrustManager::f_SetClientConnectionConcurrency(CDistributedActorTrustManager_Address _Address, int32 _ConnectionConcurrency)
	{
		...
		// Re-find after suspension: another call can remove the entry, and indexing would recreate it without a host.
	...
	// Established connections keep their current window until reconnecting.
	// Failures intentionally leave memory ahead of persistence or reconnect settings; setting the same value again is a no-op.
	// Rare partial updates are accepted to avoid rollback and retry state for overlapping setters.
	TCFuture<void> CDistributedActorTrustManager::f_SetClientConnectionSendWindow(CDistributedActorTrustManager_Address _Address, uint64 _SendWindowBytes)
	{
		...
		// Update reconnect settings; established connections keep their current window.
