...
namespace NMib::NConcurrency
{
	...
	TCFuture<void> CDistributedActorTrustManager::f_SetListenSendWindow(CDistributedActorTrustManager_Address _Address, uint64 _SendWindowBytes)
	{
		...
		// Memory first, then a copy is saved, so an overlapping setter sees this change and its
		// later save carries it; see f_SetClientConnectionConcurrency. The listen that is up keeps
		// its window until it is next started
