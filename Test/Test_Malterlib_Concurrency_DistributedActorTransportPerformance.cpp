	...
	struct CDistributedActorTransportPerformance_Tests : public NMib::NTest::CTest
	{
		...
		struct CTransportBenchState
		{
				...
				: mp_pRunLoop(_RunLoopHelper.m_pRunLoop)
			{
				...
				m_ServerTrustManager(&CDistributedActorTrustManager::f_AddListen, ServerAddress).f_CallSync(mp_pRunLoop, g_Timeout);
				...
				;
				m_ClientTrustManager(&CDistributedActorTrustManager::f_AddClientConnection, TrustTicket.m_Ticket, g_Timeout / 2, _ConnectionConcurrency)
					.f_CallSync(mp_pRunLoop, g_Timeout)
				;
		...
		static void fs_MeasureSchemes(CTestPerformance &_PerfTest, tf_FMeasure const &_fMeasure)
		{
			...
			if (!bTcp && fg_IsAuthenticatedUnixSupported() && (Schemes.f_IsEmpty() || Schemes.f_Find("wsa") >= 0))
				_fMeasure("wsa");
			if (Schemes.f_IsEmpty() || Schemes.f_Find("wss") >= 0)
				_fMeasure("wss");
		...
		void f_DoTests()
		{
			...
			DMibTestSuite(CTestCategory("TransportRemoteServe") << CTestGroup("Performance"))
			{
			...
			DMibTestSuite(CTestCategory("TransportRemoteThroughput") << CTestGroup("Performance"))
			{
